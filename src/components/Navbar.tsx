    ...
    <header className="fixed inset-x-0 top-0 z-50 px-2 pt-2.5 sm:px-4 sm:pt-3">
      <nav className="relative mx-auto flex w-full max-w-5xl items-center gap-2 rounded-3xl border border-white/20 bg-white/20 px-3 py-2 shadow-[0_18px_55px_-28px_rgba(0,0,0,0.45)] backdrop-blur-2xl backdrop-saturate-150 ring-1 ring-white/10 transition-colors duration-300 hover:border-white/30 hover:bg-white/30 dark:border-white/10 dark:bg-zinc-950/35 dark:ring-white/5 dark:hover:bg-zinc-950/45">
        ...
        <Link
          ...
          className="group flex h-10 w-10 shrink-0 items-center justify-center rounded-full border border-zinc-300 bg-white text-lg font-bold tracking-tight text-zinc-900 shadow-sm transition-all duration-300 hover:scale-105 hover:shadow-md dark:border-zinc-700 dark:bg-zinc-950 dark:text-white"
        >
        ...
        <div className="hidden flex-1 flex-wrap items-center justify-center gap-x-4 gap-y-1.5 text-[0.85rem] font-medium text-zinc-700 dark:text-zinc-300 md:flex">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`rounded-full px-3 py-1.5 transition-colors duration-300 hover:bg-white/35 hover:text-zinc-950 dark:hover:bg-white/10 dark:hover:text-white ${isActive(link.href) ? "text-zinc-950 dark:text-white" : ""
                }`}
            >
              {link.label}
            </Link>
                ...
                <div className="flex flex-col gap-1 text-sm font-medium text-zinc-700 dark:text-zinc-200">
                  {links.map((link) => (
                    <Link
                      ...
                      className={`rounded-xl px-3 py-2 transition-colors hover:bg-zinc-100 hover:text-zinc-950 dark:hover:bg-white/10 dark:hover:text-white ${isActive(link.href) ? "bg-zinc-100 text-zinc-950 dark:bg-white/10 dark:text-white" : ""
                        }`}
                    >
                      {link.label}
                    </Link>
